...
static const struct drm_display_mode default_mode = {
		...
		.hdisplay = 1080,
		.hsync_start = 1080 + 48,
		.hsync_end = 1080 + 48 + 32,
		.htotal = 1080 + 48 + 32 + 60,
		.vdisplay = 1920,
		.vsync_start = 1920 + 3,
		.vsync_end = 1920 + 3 + 5,
		.vtotal = 1920 + 3 + 5 + 6,
...
static int jdi_panel_add(struct jdi_panel *jdi)
{
	...
	print_dbg("reset gpio name %s, label %s", jdi->reset_gpio->name, jdi->reset_gpio->label);
	...
	// 	return ret;
	// }
	// print_dbg("enable gpio name %s, label %s", jdi->enable_gpio->name, jdi->enable_gpio->label);
...
static int jdi_panel_probe(struct mipi_dsi_device *dsi)
{
	...
	print_dbg("drv data set");

	jdi->dsi = dsi;

	ret = jdi_panel_add(jdi);
	print_dbg("panel add %d", ret);
	if (ret < 0)
		return ret;

	ret = mipi_dsi_attach(dsi);
	print_dbg("dsi attach %d", ret);
	return ret;
}
...
MODULE_DESCRIPTION("JDI otm1902b FHD");
